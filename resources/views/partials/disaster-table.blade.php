<div class="overflow-x-auto print-hidden">
    <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
            <tr>
                ...
                <th class="px-1 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Aftershocks</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr class="{{ $case->is_approved ? 'even:bg-gray-50 odd:bg-white hover:bg-gray-200' : 'bg-yellow-100' }} {{ $case->is_approved ? '' : 'print-hidden' }}">
                ...
                <td class="px-1 py-4 whitespace-nowrap">{{ $case->aftershocks }}</td>
                ...
                @elseif (request('type') === 'Rebel Encounter')
                <td class="px-6 py-4 whitespace-nowrap text-wrap">{{ $case->involved_parties }}</td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-wrap">{{ $case->affected_infrastructure }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-wrap">{{ $case->casualties }}</td>
                @if(auth()->user()->role === 0)
                <td class="px-6 py-4 whitespace-nowrap print-hidden">{{ $case->is_approved ? 'Approved' : 'Pending' }}</td>
    ...
    @if (request('type') && request('start_date') && request('end_date') && $totalCases > 0)
    <p class="text-lg font-medium mt-4 border-t-2 border-t-gray-300 pt-2">
        {{ __("Total Disasters for this date range: ") }} {{ $totalCases }}
    </p>
...
<!-- for print  -->
<div class="overflow-x-auto hidden print-show">
    <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
            <tr>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">NO</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @php
            // Sort disaster case: pending first, then approved
            $sortedCases = $disastersForPrint->sortByDesc(function ($case) {
            return !$case->is_approved; // Pending cases (false) come before approved (true)
            });
            @endphp
