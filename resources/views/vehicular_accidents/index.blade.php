        ...
        <div class="{{ auth()->user()->role === 0 ? 'xl:max-w-[90rem]' : 'max-w-7xl' }} mx-auto sm:px-6 lg:px-8 relative">
            <div class="bg-white overflow-hidden shadow-md sm:rounded-lg mb-1">
                                    ...
                                    <tr class="{{ $case->is_approved ? 'even:bg-gray-50 odd:bg-white hover:bg-gray-200' : 'bg-yellow-100' }} {{ $case->is_approved ? '' : 'print-hidden' }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap print-hidden">
                                            ...
                                            <form action="{{ route('vehicular_accidents.destroy', $case->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-700" onclick="return confirm('Are you sure you want to delete this vehicular accident?')">
                                                    {{ __('Delete') }}
                                                </button>
                                            </form>
                                        </td>
                ...
                <div class="p-6 text-gray-900 print-show hidden print-header">
                    ...
                    @if ($allAccidentsForPrint->count() > 0)
                    <div>
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($allAccidentsForPrint as $case)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $loop->iteration }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($case->date)->format('m/d/Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $case->rescue_team }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-wrap capitalize">{{ $case->cause_of_incident }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap capitalize">{{ $case->vehicles_involved }}</td>
                    ...
                    @elseif ($allAccidentsForPrint->isEmpty())
                    <p>{{ __("No vehicular accidents found for the selected month.") }}</p>
                    @endif
                </div>
            </div>
            ...
            fetch(`/get-barangay-accidents?month={{ request('month') }}&year={{ request('year') }}`)
                ...
                .then(data => {
                    ...
                    window.loadBarangayDetails = function(barangay, page = 1) {
                        fetch(`/get-barangay-accidents/${barangay}?month={{ request('month') }}&year={{ request('year') }}&page=${page}`)
                            .then(response => response.json())
                            .then(data => {
